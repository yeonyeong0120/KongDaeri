...
### 2.1 핵심 흐름 (모든 소스 공통)
```
캡처 → 로컬 수집함 저장 → AI 처리(분류·요약·마크다운) → '노션 공유' → 노션 페이지
```
...
ICaptureSource.Captured
        │  (CaptureItem 생성)
        ▼
  CaptureService ── 큐 ──▶ IStorage.SaveAsync   (Collected)
        │
        ▼ (백그라운드 비동기)
...
- 캡처 이벤트와 AI 처리를 **분리(큐 + async)** → AI가 실패/지연돼도 저장은 즉시 완료.
- 노션 전송 실패 시 **재시도/보류 큐**.
...
- "비밀번호 / password / OTP / 인증번호" 등 **키워드 감지** 시 해당 항목 마스킹 또는 수집 제외.
- 카드번호·이메일 등 **정규식 패턴** 마스킹.
- ※ **한계 명시**: 키워드/패턴 방식은 모든 민감정보를 잡지 못함 → 보조 수단으로 **일시정지 토글**과 **앱별 제외** 제공.
